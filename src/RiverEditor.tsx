...
import { defaultRiverLayouts, loadRiverLayouts, riverIsVfx, riverLayerIds, riverLayerStyle, saveRiverLayouts, type LayerLayout, type RiverLayerId, type RiverLayouts } from './riverbankLayout'
...
export default function RiverEditor() {
  ...
  return <main className="editor-shell"><div className="editor-preview"><div className="zoom-bar"><span>Flussufer</span><button onClick={() => zoomTo(zoom - 25)}>−</button><output>{zoom}%</output><button onClick={() => zoomTo(zoom + 25)}>+</button><button className="zoom-fit" onClick={() => zoomTo(100)}>Einpassen</button></div><div className="editor-viewport"><section className="editor-scene" style={{ width: `${zoom}%` }}><img className="scene-background" src={riverbank.background} alt="Flussufer" />{riverLayerIds.map(id => <img key={id} src={sourceFor(id)} alt={id} className={`editor-layer ${selected === id ? 'selected' : ''}`} style={{ ...riverLayerStyle(layouts[id]), mixBlendMode: riverIsVfx(id) ? 'screen' : undefined }} onPointerDown={e => down(e, id)} onPointerMove={move} onPointerUp={up} onPointerCancel={up} />)}</section></div></div><section className="editor-panel"><div className="editor-title"><div><p className="eyebrow">Scene Editor V2</p><h1>Flussufer</h1></div><button onClick={copy}>{copied ? 'Kopiert' : 'Layout kopieren'}</button></div><div className="editor-toolbar"><button onClick={undo} disabled={!history.length}>↶ Undo</button><button onClick={() => update(selected, defaultRiverLayouts[selected], true)}>Zurücksetzen</button><button className={active.visible ? 'active' : ''} onClick={() => update(selected, { visible: !active.visible }, true)}>{active.visible ? 'Sichtbar' : 'Ausgeblendet'}</button></div><div className="layer-tabs">{riverLayerIds.map(id => <button key={id} className={id === selected ? 'active' : ''} onClick={() => setSelected(id)}>{id}</button>)}</div><div className="editor-section-label">Position & Größe</div><div className="editor-controls">{basicKeys.map(control)}</div><button className="advanced-toggle" onClick={() => setAdvanced(v => !v)}><span>Perspektive & Neigung</span><span>{advanced ? '▴' : '▾'}</span></button>{advanced && <div className="editor-controls advanced-controls">{advancedKeys.map(control)}</div>}<p className="editor-help"><strong>Touch:</strong> 1 Finger verschiebt. 2 Finger skalieren und drehen. Alles wird lokal gespeichert.</p></section></main>
}
